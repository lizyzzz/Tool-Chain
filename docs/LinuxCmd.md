...
# -h：以人类可读的方式显示文件大小，即以适合阅读的单位显示文件大小。
# -s：只显示每个指定目录的总大小，而不显示其子目录的大小。
# -c：显示所有指定目录的总大小。
# -a：显示所有文件和目录的大小，包括隐藏文件和目录。
# -d <深度>：指定递归深度，即显示目录下指定层数的子目录大小。

# lsof 命令用于列出当前系统打开的文件（包括目录）、套接字（sockets）以及文件描述符的相关信息
lsof -i -n
# -c <进程名>：显示指定进程名打开的文件。
# -u <用户名>：显示指定用户打开的文件。
# -i：显示网络连接信息。
# -p <进程号>：显示指定进程号打开的文件。
# -i <协议>：仅显示指定协议（如 TCP、UDP）的网络连接信息。
# -n：不进行主机名解析，直接显示IP地址。
# -t：仅显示进程号（PID）。
